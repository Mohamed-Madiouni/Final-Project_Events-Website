...
function Activation() {
  ...
  useEffect(() => {
    if (errors.reactive) {
      M.toast({
        html: "A new validation link has been send to you",
        classes: "green",
        displayLength: 5000,
      });
      dispatch({
  ...
  return (
    <div>
      {new Date(jwt_decode(params.token).exp * 1000) > new Date() &&
        (auth.loading ? "Please wait..." : "Redirecting to...")}
      <h3>
        {new Date(jwt_decode(params.token).exp * 1000) <= new Date() &&
          "Your activation link has been expired"}
      </h3>
      {new Date(jwt_decode(params.token).exp * 1000) <= new Date() && (
        <button
          onClick={() => {
            dispatch(reactiveuser(jwt_decode(params.token)));
            setBtn(true);
          }}
          style={{
          ...
          disabled={btn}
        >
          {">>"}Click to get a new activation link{"<<"}
        </button>
      )}
    </div>
  );
