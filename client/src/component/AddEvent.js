...
const AddEvent = ({ toggle, action, setAction }) => {
  ...
  const [events, setEvents] = useState({
    ...
    description: map.inp.state
      ? map.inp.inp.description
      : action.type === "add"
      ? ""
    ...
    start: map.inp.state
      ...
      : action.type === "add"
      ? ""
      : action.payload.start.split("T")[0],
    end: map.inp.state
      ? map.inp.inp.end
      : action.type === "add"
      ? ""
      : action.payload.end.split("T")[0],
    nb_participant: map.inp.state
      ? map.inp.inp.nb_participant
      : action.type === "add"
      ? ""
      : action.payload.nb_participant,
    image: "",
    // action.type==="add"?"":action.payload.image,
    tags: [],
    time_start: map.inp.state
      ? map.inp.inp.time_start
      : action.type === "add"
      ? ""
      : action.payload.start.split("T")[1],
    time_end: map.inp.state
      ? map.inp.inp.time_end
      : action.type === "add"
      ? ""
      : action.payload.end.split("T")[1],
    error: {},
  });
  ...
  useEffect(() => {
    ...
    if (errors.success) {
      ...
      M.toast({
        html:
          action.type === "add"
            ? "Event successfully added check your list"
    ...
    if (map.inp.state)
      ...
      if (action.type === "edit")
        M.Chips.init(document.querySelectorAll(".chips"), {
          placeholder: "Optional: Press enter to add tags (3 Max)",
          limit: 3,
          data: action.payload.tags.map((el) => {
            return { tag: el };
          }),
        });
    }
  ...
  useEffect(() => {
    if (map.selected.lat) setaddress(map.selected);
  }, [map.selected]);

  const onChange_tags = (e) => {
    setEvents({
      ...events,
      [e.target.id]: [...[e.target.id], { tag: e.target.value }],
    });
  };
  const onChange = (e) => {
  ...
  const onSubmit = async (e) => {
    ...
    const newEvent = {
      ...
      image: events.image,
      tags: chip_input.current.innerText
        .replace(/\W/gi, "")
        .split("close")
        .slice(
          0,
          chip_input.current.innerText
            .replace(/\W/gi, "")
            .toLowerCase()
            .split("close").length - 1
        ),
    };
    ...
    if (action.type === "add") {
      ...
      var state = [];
      users
        .filter((el) => el.role === "administrator" || el.role === "moderator")
        .map((el) => {
    ...
    <div className="col s12">
      <div className="row">
        ...
        <form className="col s12" onSubmit={onSubmit}>
          ...
          <div className=" col s12 l6">
            <div className="input-field col s8">
              <input
                ...
                min={
                  events.start !== "" &&
                  new Date(events.start).toISOString().split("T")[0]
                }
          ...
          <div className="input-field col s12 l6 ">
            <input
              ...
              type="number"
            />
            <label htmlFor="nb_participant" className="active">
        ...
        <div className="col s12 container ">
          <h6>
            * Please note that a admin validation is required (for any{" "}
            <b> new event</b> or <b>modification</b> of an existing one).
          </h6>
        </div>
      </div>
    </div>
