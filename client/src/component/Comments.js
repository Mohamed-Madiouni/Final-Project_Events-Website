...
function Comments({ match, history }) {
  ...
  useEffect(() => {
    window.addEventListener("resize", () => {
      if (window.innerWidth < 992) setresiz(true);
      else setresiz(false);
    });
  });
  useEffect(() => {
    ...
    if (
      ...
      if (
        ...
        setRating(
          allevents
            .find((el) => el._id === match.params.event_id)
            .rating.find((el) => el.userId === auth.user._id).rate
        );
      } else {
        setdone(true);
        setstar(false);
  ...
  useEffect(() => {
    ...
    if (errors.success) setEdit("");
    if (errors.added) setComnt("");
  ...
  const onsubmit = (e) => {
    ...
    let content =
      ...
      allevents.find((el) => el._id === match.params.event_id).title;
    ...
    state = [
      ...state,
      {
        users: users.find(
          (el) =>
            el._id ===
            allevents.find((el) => el._id === match.params.event_id).id_organizer
        )._id,
        consulted: false,
      },
    ];
  ...
  const onreply = (e) => {
    ...
    let content =
      ...
      " " +
      auth.user.lname +
      " replied to a comment on your event " +
      allevents.find((el) => el._id === match.params.event_id).title;
    ...
    let content2 =
      ...
      " " +
      auth.user.lname +
      " replied to your comment on the event " +
      allevents.find((el) => el._id === match.params.event_id).title;
    ...
    state2 = [
      ...state2,
      {
        users: comments.comments.find((el) => el._id === replyid).postedBy,
        consulted: false,
      },
    ];
    ...
    <>
      {load && allevents.length !== 0 && users.length !== 0 && (
        <>
          ...
          <div
            className="row"
            style={{ marginTop: 10, filter: unfollow && "blur(3px)" }}
            onClick={(e) => {
              emoj &&
                !document.querySelector(".emoji-mart").contains(e.target) &&
                setEmoj(!emoj);
              emojedt &&
                !document.querySelector(".emoji-mart").contains(e.target) &&
                setEmojedt(!emojedt);
              emojreply &&
                !document.querySelector(".emoji-mart").contains(e.target) &&
                setEmojReply(!emojreply);
              sort &&
                !document.querySelector(".sort").contains(e.target) &&
                setsort(!sort);
              unfollow &&
                !document.querySelector(".custom_mod_com").contains(e.target) &&
                setunfollow("");
            }}
          >
            <div className="col l8 s12 ">
              <div className="comment_sec">
                <div className="com_picture">
                  <img
                    ...
                    src={
                      allevents.find((el) => el._id === match.params.event_id)
                        .image
                    }
                    width="100%"
                    height="440px"
                    ...
                    <div style={{ display: "flex", alignItems: "center" }}>
                      <div
                        style={{
                          display:
                            auth.isAuthenticated &&
                            auth.user.events.includes(
                              allevents.find(
                                (el) => el._id === match.params.event_id
                              )._id
                            ) &&
                            allevents.find(
                              (el) => el._id === match.params.event_id
                      ...
                      >
                        <i
                          className={star ? "fas fa-star" : "fas fa-star"}
                          style={{ fontSize: 25 }}
                        ></i>
                        <div
                          style={{
                      ...
                      <div
                        ...
                        onMouseLeave={() => {
                          if (!msg) {
                            setRating(0);
                            setstar(false);
                            document.querySelector(".fa-star").style.color =
                              "black";
                            document.querySelector(".rating").style.display =
                              "none";
                          }
                      ...
                      >
                        {msg ? (
                          <p style={{ color: "#2e8fa5", fontSize: 16 }}>
                      ...
                      >
                        <i
                          ...
                          style={{ color: "rgb(255, 180, 0)", fontSize: 35 }}
                        >
                          star
                        </i>
                        <div
                ...
                <div>
                  <ul className="collection org">
                    <li className="collection-item avatar">
                      ...
                      <p style={{ overflowWrap: "anywhere", marginTop: 3 }}>
                        {
                          allevents.find(
                            (el) => el._id === match.params.event_id
                          ).description
                ...
                {resiz && (
                  <ul className="collapsible colapse">
                    <li>
                      <div
                      ...
                      >
                        {" "}
                        Show events
                        <i className="material-icons right chevron">
                          expand_more{" "}
                        </i>
                      </div>
                    ...
                    {comments.comments &&
                      nbr_comments(
                        comments.comments.filter(
                          (elm) => elm.event === match.params.event_id
                        ).length
                      ) + " "}
                    comment
                    {comments.comments &&
                ...
                {auth.isAuthenticated && (
                  <form className="input_add">
                    <div style={{ position: "relative", display: "flex" }}>
                      ...
                      />
                      <div
                        style={{ width: "100%", marginTop: 6, marginLeft: 5 }}
                      >
                        <textarea
                          value={comnt}
              ...
              <button type="submit" disabled={!comnt}>Add public comment</button>
                ...
                {comments.comments &&
                  comments.comments
                    .filter((elm) => elm.event === match.params.event_id)
                    .slice(0)
                    .sort(function (a, b) {
                              ...
                              >
                                {(el.postedBy === auth.user._id ||
                                  ...
                                  <div id="editdelete">
                                    ...
                                    <i
                                      onClick={() => setDeletecomid(el._id)}
                                      className="modal-trigger material-icons"
                                      data-target="modaldeletcom"
                                      title="Delete"
                                    >
                                      delete
                                    </i>
                                ...
                                {users.find((e) => e._id === el.postedBy).role !==
                                  ...
                                  !auth.user.reports.includes(el._id) && (
                                    <span id="editdelete">
                                      <i
                                        onClick={() => setactvreport(el._id)}
                                        className="modal-trigger material-icons"
                                        data-target="modalreportcom"
                                        title="report"
                                      >
                                        report
                                      </i>
                              ...
                              >
                                <i
                                  ...
                                  style={{
                                    ...
                                    color:
                                      auth.isAuthenticated &&
                                      auth.user.likes.includes(el._id) &&
                                      "#2e8fa5",
                                  }}
                                  onClick={() => {
                                    if (actvlike) {
                                      if (
                                        ...
                                        auth.user.dislikes.includes(el._id) &&
                                          dispatch(
                                            removedislikecomment(
                                              el._id,
                                              Number(el.dislikes) - 1,
                                              auth.user._id
                                            )
                                          );
                                      } else {
                                          ...
                                          >
                                            {(el.postedBy === auth.user._id ||
                                              ...
                                              <div id="editdelete">
                                                ...
                                                <i
                                                  onClick={() =>
                                                    setDeletereplyid(el.id)
                                                  }
                                                  className="modal-trigger material-icons"
                                                  data-target="modaldeletreply"
                                                  title="delete"
                                                >
                                                  delete
                                                </i>
        ...
        <div className="modal-footer">
          <a
            ...
            onClick={() => {
              ...
              users
                .filter(
                  (el) => el.role === "administrator" || el.role === "moderator"
                )
                .map((el) => {
              ...
              dispatch(
                reportComment(
                  actvreport,
                  Number(
                    comments.comments.find((el) => el._id === actvreport).reports
                  ) + 1,
                  auth.user._id
                )
              );
            ...
            onClick={() => {
              ...
              dispatch(
                reportReply(
                  ...
                  Number(
                    comments.comments
                      .find((el) => el._id === replyid)
                      .reply.find((el) => el.id === actvreport).reports
                  ) + 1,
                  auth.user._id,
                  replyid
                )
      ...
      <div
        className="custom_mod_com"
        style={{
          display: unfollow ? "initial" : "none",
          padding: "10px",
          background: "white",
          zIndex: 10,
          border: "2px solid #2e8fa5",
        }}
      >
        <div className="modal-content">
