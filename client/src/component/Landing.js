...
import notif, {
  filter_inactive_notif,
  filter_notif,
} from "../outils/notif_length";
...
function Landing({}) {
    ...
    window.addEventListener("resize", () => {
      if (shownotif === true && notifref.current) {
        if (window.innerWidth <= 545)
          notifref.current.style.width =
            Number(Number(window.innerWidth) - 195).toString() + "px";
        else notifref.current.style.width = "350px";
      }
    });
  });
  ...
  useEffect(() => {
    if (
      userban &&
      userban.canceled === false &&
      (new Date(eventClosing(userban.created_at, userban.duration)) >
        new Date() ||
        userban.duration === -1)
    ) {
      dispatch(logoutUser());
  ...
  return (
    ...
    >
      {users.length !== 0 && (
        <div className="landing_app">
          {!localStorage.token ? (
            ...
            >
              <Link to="/register" style={{}} className="btn-nav">
                Register Now
              </Link>

              <Link to="/login" style={{}} className="btn-nav">
                Log In
              </Link>
            </div>
          ) : (
            ...
            >
              <div className="toggleNotifications">
                {notifsize > 0 && (
                  <div className="count">
                    <div className="num">
                      {allnotif.length !== 0 && notif(allnotif, auth.user._id)}
                    </div>
                  </div>
                )}
                <label
                ...
                {shownotif && (
                  ...
                  >
                    ...
                    >
                      {notifsize > 0 ? (
                        filter_inactive_notif(allnotif, auth.user._id)
                          .reverse()
                          .slice(0, notifsize > 10 ? 10 : 10)
                          .map((el, i) => {
                            return (
                              <li
                                key={i}
                    ...
                    <div
                      data-target="modalnotifuser"
                      className="btnbar modal-trigger"
                      onClick={() => {
                        notifsize > 0 &&
                          dispatch(
                            closeNotif(filter_notif(allnotif, auth.user._id))
                          );
                        dispatch({
                          type: SHOW_NOTIF,
                          payload: !shownotif,
              ...
              <a
                // href={"#"+location.pathname+"#signoutmodal"}
                href="#signoutmodal"
        ...
        <li style={{ height: "100%" }}>
          ...
          >
            ...
            >
              ...
              <span className="black-text email">{auth.user.email}</span>
            </div>
            <button
              className="account"
            ...
            >
              <i className="fas fa-cog" style={{ marginRight: 5 }}></i>Account
              setting
            </button>
            <button
              className="account"
              onClick={() =>
                history.push(`/${auth.user.role}/${auth.user._id}`)
              }
              style={{ marginBottom: "5px" }}
            >
              <i className="fas fa-address-card" style={{ marginRight: 5 }}></i>
              My profile
            </button>
          </div>
        </li>
      ...
      {chat.talk.show && (
        <div className="discumodal groupofnotes scrollbar" id="style-3">
          <Chat />
        </div>
      )}
