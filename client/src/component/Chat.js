...
function Chat() {
  ...
  const onsend = () => {
    dispatch(
      sendmessage(
        chat.discussion.find((el) => el.users.includes(chat.talk.value))._id,
        {
          text: msg.trim(),
          user: auth.user._id,
          created_at: new Date(),
  ...
  return (
    <>
      ...
      >
        {!ischat(chat.discussion, chat.talk.value, auth.user._id) ? (
          <div>
        ...
        <div className="col s8 l9">
                      ...
                      <textarea
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter")
                            msg.search(/\w/gi) !== -1 &&
                            !(
                              auth.user.blocked.includes(chat.talk.value) ||
                              users
                                .find((el) => el._id === chat.talk.value)
                                .blocked.includes(auth.user._id)
                            )
                              ? onsubmit()
                              : M.toast({
                                  html:
                                    "Action denied !! you can't send message to this person",
                                  classes: "red",
                                });
                        }}
          ...
          <div className="container">
            <div>
              <Link
                to={`/${users.find((e) => e._id === chat.talk.value).role}/${
                  users.find((e) => e._id === chat.talk.value)._id
                }`}
              >
                {" "}
                <p
                ...
                >
                  {users.find((e) => e._id === chat.talk.value).fname +
                    " " +
                    users.find((e) => e._id === chat.talk.value).lname}
                </p>
              </Link>
            </div>
            {chat &&
              chat.discussion
                .find(
                  (el) =>
                    el.users.includes(chat.talk.value) &&
                  ...
                  return (
                    ...
                    >
                      <li
                        ...
                        style={{
                          background:
                            el.user === auth.user._id ? "#595958" : "#55a6f2",
                          color: "white",
                        }}
                      >
                        <div
                        ...
                        >
                          <div>
                            <Link
                              to={`/${
                                users.find((e) => e._id === el.user).role
                              }/${users.find((e) => e._id === el.user)._id}`}
                            >
                              <img
                                src={users.find((e) => e._id === el.user).avatar}
                                alt=""
                                className="circle"
                              />
                            </Link>
                        ...
                        <p
                          style={{
                            ...
                            textDecorationLine: el.delete && "line-through",
                          }}
                        >
                          {el.text}
                        </p>
