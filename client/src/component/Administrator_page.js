...
<div className=" row" style={{verticalAlign: "middle",margin:"30px 15px 20px 15px"
      ...
      <div className=" col s12 organizer_hi ">
          {users.length!=0&& <div style={{width:"100%",display:"flex",justifyContent:"center",alignItems:"center"}}>
               ...
               { (auth.user.blocked&&auth.user._id!=match.params.administratorId&&!(auth.user.blocked.includes(match.params.administratorId))||users.find(el=>el._id==match.params.administratorId).blocked.includes(auth.user._id))&&<i
