...
export const deleteEvent = (idEvent) => (dispatch) => {
  setAuthToken(localStorage.token);
  axios
    .delete(`admin/events/delete/${idEvent}`)
    .then((res) => {
      dispatch(getEvents());
    })
...
//Ban user
export const banUser = (email, type, duration, reason, author) => (
  dispatch
) => {
  setAuthToken(localStorage.token);
  axios
    .post("admin/sanction/ban/add", {
      email: email,
      type: type,
      duration: duration,
      reason: reason,
...
export const unbanUser = (IdBan, cancelreason, cancelauthor) => (dispatch) => {
  setAuthToken(localStorage.token);
  axios
    .put(`admin/sanction/ban/delete/${IdBan}`, {
      canceled: true,
      cancelreason: cancelreason,
      cancelauthor: cancelauthor,
...
//Alert user
export const alertUser = (email, type, duration, reason, author) => (
  dispatch
) => {
  setAuthToken(localStorage.token);
  axios
    .post("admin/sanction/alert/add", {
      email: email,
      type: type,
      duration: duration,
      reason: reason,
...
//Remove alert user
export const unalertUser = (IdAlert, cancelreason, cancelauthor) => (
  dispatch
) => {
  setAuthToken(localStorage.token);
  axios
    .put(`admin/sanction/alert/delete/${IdAlert}`, {
      canceled: true,
      cancelreason: cancelreason,
      cancelauthor: cancelauthor,
      cancelled_at: new Date(),
...
export const validateEvent = (idEvent, valid) => (dispatch) => {
  setAuthToken(localStorage.token);
  axios
    .put(`admin/events/valid/${idEvent}`, valid)
    .then((res) => {
      dispatch(getEvents());
      dispatch({
        type: GET_ERRORS,
...
export const invalidateEvent = (idEvent, unvalid) => (dispatch) => {
  setAuthToken(localStorage.token);
  axios
    .put(`admin/events/invalid/${idEvent}`, unvalid)
    .then((res) => {
      dispatch(getEvents());
      dispatch({
        type: GET_ERRORS,
...
export const AddModo = (idUser, moderator) => (dispatch) => {
  setAuthToken(localStorage.token);
  axios
    .put(`admin/users/addmodo/${idUser}`, moderator)
    .then((res) => {
      dispatch(getUsers());
      dispatch({
        type: GET_ERRORS,
...
export const DeleteModo = (idUser, participant) => (dispatch) => {
  setAuthToken(localStorage.token);
  axios
    .put(`admin/users/removemodo/${idUser}`, participant)
    .then((res) => {
      dispatch(getUsers());
      dispatch({
        type: GET_ERRORS,
